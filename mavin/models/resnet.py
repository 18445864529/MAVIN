...
class ResnetBlock2D(nn.Module):
    ...
    def forward(
        ...
        if temb is not None and self.time_embedding_norm == "default":
            hidden_states = hidden_states + temb  # temb: bf c 1 1
...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(
        ...
        # conv layers  !!!  modified to causal conv
        pad_size = 1
        self.conv1 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, in_dim),
            nn.SiLU(),
            nn.Conv3d(in_dim, out_dim, (3, 1, 1), padding=(pad_size, 0, 0)),
        )
        self.conv2 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, out_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Conv3d(out_dim, in_dim, (3, 1, 1), padding=(pad_size, 0, 0)),
        )
    ...
    def forward(self, hidden_states: torch.Tensor, num_frames=1, cross_attention_kwargs=None) -> torch.Tensor:
        hidden_states = hidden_states[None, :].reshape((-1, num_frames) + hidden_states.shape[1:]).permute(0, 2, 1, 3, 4)  # b c f h w
